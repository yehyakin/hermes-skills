...
triggers:
  ...
  - 直播话术提取
  - 竞品视频分析
tags: [Whisper, 视频转写, 切片, FFmpeg, 电商, 直播, 字幕]
---
...
## 步骤 8：完成后自动存入 neirong-fuoli 素材库 ⭐（新增关键步骤）
...
```
~/content-fuoli/
├── 03-素材库/爆款话术库/     ← 高价值话术文本
│   └── YYYYMMDD-{竞品名}-{片段名}.md
├── 06-竞品监控/竞品分析/     ← 完整竞品分析报告
│   └── YYYYMMDD-{竞品名}-竞品分析.md
└── 04-数据复盘/             ← 切片统计
    └── content_data_table.md（追加记录）
```
...
**话题标签**：#{tag1} #{tag2}
**话术亮点**：{3-5句核心话术摘录}
**可复用结构**：{如"FABE法则/痛点+解决方案/对比型"等}
**入库时间**：{日期}
**标签**：#竞品 #话术 #{竞品名} #高光片段
```

**追加到数据表：**
在 `04-数据复盘/content_data_table.md` 追加一行：
`| {日期} | {片段名} | 抖音 | {时长}秒 | - | - | {话题标签} |`
